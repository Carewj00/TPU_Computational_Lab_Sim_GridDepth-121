...
The ultimate test of TPU theory - from substrate to cosmic web (FIXED VERSION)
"""
...
class TPU_L6_Simulation:
    def __init__(self, l0_l1_data_dir="tpu_output_V56c_auto", l2_data_dir="L2_excitations", 
        ...
        self.cosmic_grid_size = 64     # Reduced for stability
        self.hubble_time = 13.8e9      # Years
        self.simulation_steps = 10000  # Reduced steps for stability
    ...
    def create_dark_matter_field(self):
        ...
        num_dm_halos = 500  # Reduced for smaller grid
        ...
        for i in range(num_dm_halos):
            ...
            halo_radius = 5  # Grid cells (reduced)
    ...
    def create_baryon_field(self):
        ...
        num_galaxies = 100  # Reduced for smaller grid
        
        for i in range(num_galaxies):
            # Random position
            x_pos = np.random.randint(5, self.cosmic_grid_size-5)
            y_pos = np.random.randint(5, self.cosmic_grid_size-5)
            z_pos = np.random.randint(5, self.cosmic_grid_size-5)
            ...
            galaxy_radius = 3  # Grid cells (reduced)
    ...
    def compute_hubble_parameter(self, cosmic_sigma, scale_factor):
        ...
        # TPU Hubble parameter with stability
        H_0 = 70.0  # km/s/Mpc
        
        # Ensure positive values and reasonable scaling
        safe_sigma = np.maximum(mean_sigma, 1e-6)
        safe_scale_factor = np.maximum(np.abs(scale_factor), 0.1)
        
        hubble = H_0 * safe_sigma / (1.0 + safe_scale_factor)
    ...
    def evolve_cosmic_fields(self, dt):
        ...
        # Add tiny amount of clustering
        if np.random.rand() < 0.001:  # Very rare evolution
            laplacian_dm = self.compute_laplacian_3d(self.dark_matter_density)
            self.dark_matter_density += dt * 1e-8 * laplacian_dm
            self.dark_matter_density = np.maximum(self.dark_matter_density, 0)
        
        # Dark energy evolution (L2 fields)
        # Dark energy slowly increases (cosmic acceleration)
        self.dark_energy_density *= (1 + 1e-8 * dt)  # Much slower evolution
        
        # Baryon evolution (L4/L5 matter)
        # Baryons cluster under gravity but slowly
        if np.random.rand() < 0.01:  # Occasional clustering
            laplacian_baryons = self.compute_laplacian_3d(self.baryon_density)
            self.baryon_density += dt * 1e-6 * laplacian_baryons
            self.baryon_density = np.maximum(self.baryon_density, 0)
        
        # Cosmic σ evolution - very slow and stable
        # σ decreases with cosmic expansion (sampling becomes sparser)
        expansion_factor = 1 - 1e-8 * dt  # Much slower
        self.cosmic_sigma *= expansion_factor
        self.cosmic_sigma = np.maximum(self.cosmic_sigma, 1e-6)  # Prevent collapse
        
        # Substrate evolution (L0/L1 is eternal but can have tiny fluctuations)
        if np.random.rand() < 0.0001:  # Very rare fluctuations
            substrate_fluctuations = np.random.normal(0, 1e-8, self.cosmic_substrate.shape)
    ...
    def compute_power_spectrum_3d(self, field):
        ...
        # Radial average
        k_bins = np.logspace(-3, 0, 20)  # Reduced bins for smaller grid
    ...
    def run_cosmological_evolution(self):
        ...
        for step in range(self.simulation_steps):
            ...
            # Update scale factor from Hubble parameter (more stable integration)
            current_hubble = self.compute_hubble_parameter(self.cosmic_sigma, scale_factor)
            
            # Friedmann equation: da/dt = H * a, but with TPU modifications
            # Use smaller time steps and ensure stability
            da_dt = current_hubble * scale_factor * 1e-3  # Reduced rate for stability
            scale_factor += dt * da_dt
            
            # Ensure scale factor stays positive and reasonable
            scale_factor = np.maximum(scale_factor, 0.1)
            scale_factor = np.minimum(scale_factor, 10.0)
